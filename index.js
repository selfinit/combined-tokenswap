...
const ALCHEMIX_VAULT_ABI = require("./abis/alchemixVault.json"); // Import Alchemix Vault ABI
...
const ALCHEMIX_VAULT_ADDRESS = "0x64e3a05E6508558C9F3d7D2F7b9B6a7002Ff039b"; 
...
async function supplyAndBorrowAlchemix(tokenAddress, amount, signer) {
  try {
    // Approve Alchemix Vault to spend your tokens
    const tokenContract = new ethers.Contract(tokenAddress, TOKEN_IN_ABI, signer);
    const approveTransaction = await tokenContract.approve.populateTransaction(
    ...
    // Supply tokens to Alchemix Vault
    ...
    throw new Error("Alchemix supply failed");
  }
}
